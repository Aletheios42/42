...
int handle_height(char **line, t_map *map, int fd) {
  ...
  map->map[map->dim.rows][map->dim.cols[map->dim.rows]][0] = result;
...
int handle_color(char **line, t_map *map, int fd) {
  ...
  // Guardamos el resultado en map->map[row][col]
  map->map[map->dim.rows][map->dim.cols[map->dim.rows]][1] = result;
...
int handle_newline(char **line, t_map *map, int fd) {
  ...
  *line = get_next_line(fd);
  ...
  map->dim.cols[map->dim.rows] = columns; // Guardamos el número de columnas
